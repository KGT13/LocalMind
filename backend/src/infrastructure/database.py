...
def get_collections():
    ...
    ollama_ef = _get_embedding_function()
...
def get_chunks_by_source(filename):
    collection = get_collections()
    results = collection.get(where={"source":filename}, limit=10000)
    logger.info(f"Amount of chunks exctracted from source: {len(results.get('ids', []))}")
    return results
