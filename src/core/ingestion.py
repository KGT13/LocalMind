...
def ingest_file(file_path):
    ...
    if database.document_exists(name):
        ...
        return {"filename": name, "pages": 0, "chunks_stored": 0, "message": "File already exist"}
    else:
...
def ingest_text(text, source_label):
    ...
    if database.document_exists(source_label):
        logger.warning("File already exist")
        return {"filename": source_label,"pages": 0, "chunks_stored": 0, "message": "File already exist"}
    else:
